#!/usr/bin/env python3
"""Build the GitHub Pages SCAD manifest with recursive include/use closures."""
...
def main() -> int:
    ...
    def closure(rel: str) -> list[str]:
        ...
        # Safety first: if a dynamic/external include cannot be resolved, mount all source.
    ...
    if unresolved:
        print("Unresolved external includes; affected entries fall back to all sources:")
    ...
    sizes = [len(e["dependencies"]) for e in entries]
    print(f"manifest: {len(files)} files, {len(entries)} entries; closure min={min(sizes)}, max={max(sizes)}")
    return 0

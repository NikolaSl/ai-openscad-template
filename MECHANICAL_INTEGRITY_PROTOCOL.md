...
This protocol is project-agnostic. It complements `DESIGN_PROTOCOL.md`, `VISUAL_QA_PROTOCOL.md`, and `MOTION_QA_PROTOCOL.md`.
...
- use state parameters to test collision/clearance only for the coordinates that are actually constrained by geometry;
- account for any remaining uncontrolled rotations/translations with conservative envelopes or explicit setup instructions;
- verify the final retained/clamped state separately.

Example: a single screw moving in a slot constrains the **screw center** to the slot path, but an attached payload can still yaw around that screw while loose. That is a manual adjustment state, not a self-guided one-DOF carriage. If repeatable/autonomous one-dimensional motion is required, add a second guide, keyed carriage, rail, anti-rotation feature, linkage or equivalent real constraint and re-run the constraint/state-space QA.
